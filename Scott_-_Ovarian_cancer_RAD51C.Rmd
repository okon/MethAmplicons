...
```{r read_Supporting_Files, echo = FALSE, eval=FALSE}
gene_HRD_short<-read.csv("data/supporting/181212_Pollock_HRD_list_short.csv", header = F, 
                         stringsAsFactors = F)[,1]
gene_BROCA_full<-read.csv("data/supporting/BROCA_v6_full.csv", header = F, stringsAsFactors = F)[,1]
gene_BROCA_short<-read.csv("data/supporting/BROCA_v6_short.csv", header = F, stringsAsFactors = F)[,1]
gene_Cosmic_Census<-read.csv("data/supporting/cancer_gene_census.csv", header = T, stringsAsFactors = F)[,1]
wgs_meta<-read.table("data/supporting/Scott_WGS_metadata.tsv", header = T, stringsAsFactors = F, sep ="\t") %>%
...
#Matched BL and PDX sample models
matched_models <- c("PH039","134011","134013","134019","134091",
                    "134056","134062","134183","23133","134027","134148",
                    "134169","134143","134048")

```

```{r add_hr_status_to_meta, eval=FALSE}
wgs_meta %<>%
  mutate(HR_status = case_when(DonorLabel %in% c("PH039","134011","134013","134019","134091",
                                                 "134056","134062","134183","23133") ~ "HR-D",
                               DonorLabel %in% c("134036","134027","134029","134148","134111") ~ "HR-C",
                               DonorLabel %in% c("134169","134143","134048") ~ "HR-R")) %>%
  mutate(HR_status = factor(HR_status, levels = c("HR-D","HR-R","HR-C")))
...
```{r load_meth_amplicon_data, message=FALSE, warning=FALSE}
...
```{r load_ICGC_metadata, message=FALSE, warning=FALSE}
...
```{r load_ICGC_array_summary_stats, message=FALSE, warning=FALSE}
...
```{r HRD_analysis_scarHRD_ICGC_array, message=FALSE, warning=FALSE}
...
```{r HRD_analysis_scarHRD_ICGC_WGS, message=FALSE, warning=FALSE}
